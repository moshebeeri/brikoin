...
export function withProjectTradesStats(WrappedComponent) {
  class ProjectTradesStats extends React.PureComponent {
    constructor(props) {
      super(props);
    ...
    render() {
      ...
      if (!this.dataLoaded()) {
        return <LoadingCircular open />;
      }
      if(this.props.initDoneFunction){
        this.props.initDoneFunction(true)
      }
      ...
      const filteredAsks =
        currentProjectAsks && currentProjectAsks.length > 0
          ? currentProjectAsks.filter(ask => ask.state === "initial"  &&  ask.active)
          : [];
  ...
  return withProject(
    connect(mapStateToProps, mapDispatchToProps)(ProjectTradesStats)
  );
}
