...
class ReactImageUploadComponent extends React.Component {
  ...
  componentDidMount () {
    if (this.props.fieldValue) {
      this.setState({
        pictures: this.props.fieldValue
      })
    }
  }
  ...
  renderPreviewPictures () {
    ...
    return this.state.pictures.map((picture, index) => {
      return (
        <div key={index} className={direction === 'rtl' ? 'uploadPictureContainer' : 'uploadPictureContainerLtr'}>
          <IconButton style={{marginTop: -45}} onClick={this.removeImage.bind(this, picture)}>
  ...
  getComponentHeight () {
    ...
    if (this.state.pictures.length > 0 && this.state.pictures.length < 3) {
      return 300
    }
    if (this.state.pictures.length >= 3 && this.state.pictures.length < 5) {
      return 450
    }
  ...
  render () {
    ...
    return (
      <div style={{height: this.getComponentHeight(), width: '100%'}}>
        <div className={'fileUploader ' + this.props.className} style={this.props.style}>
          <div className='fileContainer' style={this.props.fileContainerStyle}>
            ...
            <div style={{marginLeft: 50, marginRight: 50}}>
              <Button fullWidth variant='outlined' className={classes.button}
                onClick={this.triggerFileUpload.bind(this)}
          >
                {this.props.t('Choose Pictures')}
          ...
          />
            {this.state.pictures.length > 0 && <IconButton style={{
              zIndex: 22,
              marginTop: -45,
              position: 'relative',
              left: direction === 'rtl' ? -210 : 210,
              top: 5}} onClick={this.showPreview.bind(this)}>
              { this.state.showPreview ? <KeyboardArrowUp style={{fontSize: 20}} /> : <KeyboardArrowDown style={{fontSize: 20}} />}
            </IconButton>}
...
export default withCusomeStyle(connect(
