...
import { addGroupVotingRights } from "../../redux/actions/groups";

export function GroupAddVotingRights(props) {
...
function addVotingRights(props, user, setSearch, setUsers) {
  props.addGroupVotingRights(
    user.userId,
    getMemberId(props, user),
    props.group.id
  );
  setSearch("");
  setUsers([]);
...
function getMemberId(props, user) {
  let ids = Object.keys(props.group.members).filter(
    memberKey => props.group.members[memberKey].userId === user.userId
  );
  if (ids.length > 0) {
    return ids[0];
  }
...
const mapDispatchToProps = {
  addGroupVotingRights
};
export default withWidth()(
  withUser(connect(mapStateToProps, mapDispatchToProps)(GroupAddVotingRights))
);
