const pledgeeFields = require(`./pledgee.json`);
...
const listItems = Object.keys(pledgeeFields).map(field => {
  return (
    `<div style={{flex: 1}}>                         \n` +
    `<TextField label='${pledgeeFields[field].name}'   \n` +
    `    id='${field}'                                    \n` +
    `    className={classes.textField}                    \n` +
    `    onChange={this.handleChange('${field}')}         \n` +
    `    margin='normal'                                  \n` +
    `    fullWidth                                        \n` +
    `    type='${toType(pledgeeFields[field].type)}'   \n` +
    `  />                                                 \n` +
    `</div>                                               \n`
  );
});
