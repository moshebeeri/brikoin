...
import { askMortgage } from "../../redux/actions/mortgage";
...
class MortgageView extends React.Component {
  ...
  askMortgage() {
    const { askMortgage, mortgage, projectAddress, user } = this.props;
    // TODO validate terms
    if (this.state.amount && this.state.mortgageType) {
  ...
  mortgageTerms(logo, classes, mortgage, availableFounds) {
    return (
      ...
      >
        ...
        <div style={{ marginLeft: 10, marginRight: 10, fontWeight: "bold" }}>
          {this.props.t("availableFounds")} :{" "}
          {availableFounds.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, "$&,")} $
        </div>
      </div>
    );
  }
