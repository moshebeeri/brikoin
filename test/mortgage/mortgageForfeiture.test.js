...
contract("Mortgage Forfeiture", function(accounts) {
  let cornerStone;
  let mortgageFactory;
  ...
  it("Forfeiture  ", async () => {
    ...
    let remainingMortage = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    ...
    console.log("user1Holdings before payment " + user1Holdings);
    console.log("user2Holdings before payment " + user2Holdings);
    console.log("remainingMortage before payment1 " + remainingMortage);
    await cornerStone.mortgagePayment(mortgageAddress, MILL, { from: user0 });
    user1Holdings = await cornerStone.getUserHoldings(
      testStoneCoinAddress,
      user1
    );
    remainingMortage = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    ...
    console.log("remainingMortage after payment 1 " + remainingMortage);
    let forfeitureHoldings = Math.floor(paymentInDollar * 1.1);
    console.log("ForfeitureHoldings" + forfeitureHoldings);
    console.log("user1Holdings after payment1 " + user1Holdings);
    console.log("user2Holdings after payment1 " + user2Holdings);
    assert.equal(parseInt(user1Holdings), forfeitureHoldings);
    assert.equal(parseInt(user2Holdings), 150000 - forfeitureHoldings);
    await cornerStone.mortgagePayment(mortgageAddress, MILL, { from: user0 });
    remainingMortage = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    console.log("remainingMortage " + remainingMortage);
    user1Holdings = await cornerStone.getUserHoldings(
      testStoneCoinAddress,
      user1
    );
    ...
    console.log("user1Holdings after payment2 " + user1Holdings);
    console.log("user2Holdings after payment2 " + user2Holdings);
    console.log("remainingMortage after payment2 " + remainingMortage);
    assert.equal(parseInt(user1Holdings), 2 * forfeitureHoldings);
    assert.equal(parseInt(user2Holdings), 150000 - 2 * forfeitureHoldings);
    await cornerStone.mortgagePayment(mortgageAddress, MILL, { from: user0 });
    ...
    console.log("user1Holdings after payment3 " + user1Holdings);
    console.log("user2Holdings after payment3 " + user2Holdings);
    console.log("remainingMortage after payment3 " + remainingMortage);
    let forfeitureRemainigngHoldings = Math.floor(
      (remainingMortage / 1000000) * 1.1
    );
    assert.equal(
      parseInt(user2Holdings),
      150000 - 2 * forfeitureHoldings - forfeitureRemainigngHoldings
    );
    remainingMortage = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    assert.equal(parseInt(remainingMortage), 0);
    assert.equal(true, true);
  });
});
