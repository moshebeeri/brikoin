...
contract("Mortgage Clearances With Doc", function(accounts) {
  let cornerStone;
  let mortgageFactory;
  ...
  before(async () => {
    ...
    await cornerStone.initialAsk(testStoneCoinAddress, 150000, { from: user0 });
  ...
  // it('should fail bid  ', async () => {
  //   try {
  //     await cornerStone.bidMortgaged(mortgageeRequestAddress,
  //     testStoneCoinAddress,
  //     50000 * MILLION,
  //     150000,
  //     MILLION,
  ...
  //     assert(false,  true)
  //   } catch (error) {
  //     assert.equal(error.message, 'VM Exception while processing transaction: revert', 'Ask transaction should not be created')
  //   }
  // })

  it("should do bid  ", async () => {
    ...
    let signedDocument = signedDocumentCreated[0].args["SignedDocument"];
    await signedDocumentFactory.shouldSign(signedDocument, user0, "ADMIN", {
      from: user0
    });
    await signedDocumentFactory.shouldSign(signedDocument, user2, "USER", {
      from: user2
    });
    await signedDocumentFactory.sign(signedDocument, { from: user0 });
  ...
  it("Ask should fail  mortgage ", async () => {
    try {
  ...
  it("Ask mortgage ", async () => {
    ...
    mortgageTradeKey = new Date().getTime();
    // let user3Holdings = await cornerStone.getUserHoldings(testStoneCoinAddress, user3)
    // let user2Holdings = await cornerStone.getUserHoldings(testStoneCoinAddress, user2)
    // let user0Holdings = await cornerStone.getUserHoldings(testStoneCoinAddress, user0)
    // console.log('user3Holdings ' + user3Holdings + ' user2Holdings ' + user2Holdings + ' user0Holdings ' + user0Holdings)
    await cornerStone.askMortgaged(
      mortgageAddress,
      testStoneCoinAddress,
      40000,
    ...
    await signedDocumentFactory.shouldSign(signedDocument, user3, "USER", {
      from: user3
    });
    await signedDocumentFactory.sign(signedDocument, { from: user0 });
    await signedDocumentFactory.sign(signedDocument, { from: user3 });
    ...
    assert.equal(user3Holdings, 40000);
    assert.equal(user2Holdings, 110000);
    ...
    let mortgageScheduledPayment =
      PAYMENTS.paymentSchedule[2].paymentTotal * MILLION;
    let mortgageRemaining =
      PAYMENTS.paymentSchedule[2].remainingLoanBalnce * MILLION;
    let shouldBe =
      80000 * MILLION - (mortgageScheduledPayment + mortgageRemaining);
