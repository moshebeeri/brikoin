...
contract("Mortgagee", function(accounts) {
  ...
  it("create mortgage request", async () => {
    ...
    mortgageeRequest = await mortgageRequestFactory.createExternalMortgageRequest(
      user2,
  ...
  it("create mortgage ", async () => {
    ...
    mortgageResult.addOwnership(user0, { from: user0 });
  ...
  it("test default ", async () => {
    ...
    let mortgage = Mortgage.at(mortgageAddress);
    await mortgage.externalDefaulted({ from: user0 });
    let status = await mortgage.getDefaultStatus({ from: user0 });
    assert.equal(1, parseInt(status));
  });
});
