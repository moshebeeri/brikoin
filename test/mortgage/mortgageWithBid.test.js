...
contract("Mortgage With bid", function(accounts) {
  let cornerStone;
  let mortgageFactory;
  ...
  before(async () => {
    ...
    mortgageOperationAddress = await mortgageOperation.getAddress();
    ...
    await cornerStone.initialAsk(testStoneCoinAddress, 320000, { from: user0 });
    await cornerStone.deposit(user1, 3000000 * MILLION);
    await cornerStone.deposit(user2, 150000 * MILLION);
  ...
  it("Simple bid  ", async () => {
    await cornerStone.bid(testStoneCoinAddress, 20000, MILLION, 0, {
      from: user3
    });
  ...
  it("Simple ask and trade  ", async () => {
    await cornerStone.ask(testStoneCoinAddress, 10000, 1.1 * MILLION, 0, {
      from: user3
    });
    await cornerStone.bidCancel(testStoneCoinAddress, { from: user2 });
    await cornerStone.bid(testStoneCoinAddress, 10000, 1.1 * MILLION, 0, {
      from: user2
    });
    await cornerStone.trade(
      testStoneCoinAddress,
  ...
  it("check balance  user2 ", async () => {
    let balance = await cornerStone.balanceOf(user2, { from: user2 });
