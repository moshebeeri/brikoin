...
contract("Mortgage clear", function(accounts) {
  let cornerStone;
  let mortgageFactory;
  ...
  before(async () => {
    ...
    await cornerStone.deposit(user2, 1000000 * MILLION);
  ...
  it("clear  ", async () => {
    ...
    await cornerStone.clearAllMortgage(mortgageAddress, { from: user2 });
    let mortgageRemaining = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    assert.equal(parseInt(mortgageRemaining), 0);
  });
});
