...
const REFINANCE = {
  ...
  startDate: "2018-09-01",
  loanAmount: 27350.49,
  totalLoanCost: 143.68,
  paymentSchedule: [
    {
      interestRate: 4.2,
      scheduledMonthlyPayment: 13747.08,
      interest: 95.73,
      principal: 13651.36,
      extra: 0,
      principalTotal: 13651.36,
      propertyTax: 0,
      paymentTotal: 13747.08,
      paymentDate: "2018-10-01T00:00:00.000Z",
      remainingLoanBalnce: 13699.13,
      loanMonth: 1,
      loanYear: 1,
      interestLoanYearToDate: 95.73,
      principalLoanYearToDate: 13651.36,
      extraLoanYearToDate: 0,
      principalTotalLoanYearToDate: 13651.36,
      propertyTaxLoanYearToDate: 0,
      paymentTotalLoanYearToDate: 13747.08,
      interestToDate: 95.73,
      principalToDate: 13651.36,
      extraToDate: 0,
      principalTotalToDate: 13651.36,
      propertyTaxToDate: 0,
      paymentTotalToDate: 13747.08
    },
    {
      interestRate: 4.2,
      scheduledMonthlyPayment: 13747.08,
      ...
      principal: 13699.13,
      extra: 0,
      principalTotal: 13699.13,
      propertyTax: 0,
      paymentTotal: 13747.08,
      paymentDate: "2018-11-01T01:00:00.000Z",
      remainingLoanBalnce: 0,
      loanMonth: 2,
      loanYear: 1,
      interestLoanYearToDate: 143.67,
      principalLoanYearToDate: 27350.49,
      extraLoanYearToDate: 0,
      principalTotalLoanYearToDate: 27350.49,
      propertyTaxLoanYearToDate: 0,
      paymentTotalLoanYearToDate: 27494.16,
      interestToDate: 143.67,
      principalToDate: 27350.49,
      extraToDate: 0,
      principalTotalToDate: 27350.49,
      propertyTaxToDate: 0,
      paymentTotalToDate: 27494.16
    }
  ],
  numberOfPayments: 2,
  monthlyPayment: 13747.08
};
...
contract("Mortgage Re-finance", function(accounts) {
  let cornerStone;
  let mortgageFactory;
  let testStoneCoinAddress;
  let totalSystemFounds;
  ...
  const paymentRefinance =
    REFINANCE.paymentSchedule[0].scheduledMonthlyPayment * MILLION;
  ...
  before(async () => {
    ...
    totalSystemFounds = await cornerStone.getOwnerBalance();
    ...
    await cornerStone.initialAsk(testStoneCoinAddress, 150000, { from: owner });

    await cornerStone.deposit(user1, 300000 * MILLION, { from: owner });
    await cornerStone.deposit(user2, 200000 * MILLION, { from: owner });
    await cornerStone.deposit(user3, 200000 * MILLION, { from: owner });
  ...
  it("Payment ", async () => {
    ...
    await cornerStone.mortgagePayment(mortgageAddress, MILL, { from: owner });
    ...
    assert.equal(
      parseInt(payment) * 2 + parseInt(balance),
      parseInt(beforeBalance)
    );
  });

  it("Ask mortgage ", async () => {
    mortgageTradeKey = new Date().getTime();
    let mortgageContract = Mortgage.at(mortgageAddress);
    let remainingMortgage = await mortgageContract.getRemainingLoanBalance({
      from: user2
    });
    console.log("remainingMortgageBefore: " + remainingMortgage);
    ...
    await cornerStone.bid(testStoneCoinAddress, 5000, 1.3 * MILLION, 0, {
      from: user3
    });
    await cornerStone.createTradeMortgage(
      mortgageTradeKey,
    ...
    let remainingMortgageAfterCleararnce = await mortgageContract.getRemainingLoanBalance(
      { from: user2 }
    );
    console.log(
      "remainingMortgageAfterCleararnce: " + remainingMortgageAfterCleararnce
    );
    assert.equal(
      parseInt(remainingMortgage),
      parseInt(remainingMortgageAfterCleararnce) + 5000 * 1.3 * MILLION
    );
    assert.equal(parseInt(user3Holdings), 5000);
    assert.equal(parseInt(user2Holdings), 145000);
    let user2Balance = await cornerStone.balanceOf(user2);

    assert.equal(parseInt(user2Balance), parseInt(balanceLast));
  });
  it("Ask mortgage2 ", async () => {
    mortgageTradeKey = new Date().getTime();
    await cornerStone.askMortgaged(
      mortgageAddress,
      testStoneCoinAddress,
    ...
    let mortgaeContract = Mortgage.at(mortgageAddress);
    let shouldRefinance = await mortgaeContract.shouldRefinance();
    ...
    assert.equal(user3Holdings, 10000);
    assert.equal(user2Holdings, 140000);
    let user2Balance = await cornerStone.balanceOf(user2);

    assert.equal(parseInt(user2Balance), parseInt(balanceLast));
  });
  it("Payment with clearance ", async () => {
    let beforeBalance = await cornerStone.balanceOf(user2);
    let user1BeforeBalance = await cornerStone.balanceOf(user1);
    await cornerStone.mortgagePayment(mortgageAddress, MILL * 1.3, {
      from: owner
    });
    let mortgaeContract = Mortgage.at(mortgageAddress);
    let shouldRefinance = await mortgaeContract.shouldRefinance();
    ...
    console.log("beforeBalanceUser2 " + balance);
    console.log("user1BeforeBalance " + user1BeforeBalance);
    console.log("user1Balance " + user1Balance);
    let mortgageContract = Mortgage.at(mortgageAddress);
    let refinanceSum = await mortgageContract.getRemainingLoanBalance({
      from: owner
    });

    console.log("refinanceSum: " + refinanceSum);

    assert.equal(true, true);
  });
  it("Mortgage Refinance ", async () => {
    let mortgageContract = Mortgage.at(mortgageAddress);
    let refinanceSum = await mortgageContract.getRemainingLoanBalance({
      from: owner
    });
    let lastPayedIndex = await mortgageContract.getNextPaymentIndex({
      from: owner
    });
    console.log("refinanceSum: " + refinanceSum);
    console.log("lastPaiedIndex: " + lastPayedIndex);
    await mortgageContract.addPaymentSchedule(
      parseInt(0) + parseInt(lastPayedIndex),
      REFINANCE.paymentSchedule[0].interestRate,
      REFINANCE.paymentSchedule[0].scheduledMonthlyPayment * MILLION,
      REFINANCE.paymentSchedule[0].interest * MILLION,
      REFINANCE.paymentSchedule[0].principal * MILLION,
      REFINANCE.paymentSchedule[0].principalTotal * MILLION,
      REFINANCE.paymentSchedule[0].propertyTax,
      REFINANCE.paymentSchedule[0].paymentTotal * MILLION,
      new Date(REFINANCE.paymentSchedule[0].paymentDate).getTime(),
      REFINANCE.paymentSchedule[0].remainingLoanBalnce * MILLION,
      REFINANCE.paymentSchedule[0].loanMonth,
      REFINANCE.paymentSchedule[0].loanYear,
      { from: owner }
    );

    await mortgageContract.addPaymentSchedule(
      parseInt(1) + parseInt(lastPayedIndex),
      REFINANCE.paymentSchedule[1].interestRate,
      REFINANCE.paymentSchedule[1].scheduledMonthlyPayment * MILLION,
      REFINANCE.paymentSchedule[1].interest * MILLION,
      REFINANCE.paymentSchedule[1].principal * MILLION,
      REFINANCE.paymentSchedule[1].principalTotal * MILLION,
      REFINANCE.paymentSchedule[1].propertyTax,
      REFINANCE.paymentSchedule[1].paymentTotal * MILLION,
      new Date(REFINANCE.paymentSchedule[1].paymentDate).getTime(),
      REFINANCE.paymentSchedule[1].remainingLoanBalnce * MILLION,
      REFINANCE.paymentSchedule[1].loanMonth,
      REFINANCE.paymentSchedule[1].loanYear,
      { from: owner }
    );
    assert.equal(true, true);
  });

  it("Payment After refinance ", async () => {
    let stoneCoinContract = StoneCoin.at(testStoneCoinAddress);
    userOwnedHoldings = await stoneCoinContract.getBalance(user2);
    let mortgageContract = Mortgage.at(mortgageAddress);
    assert.equal(parseInt(userOwnedHoldings), 0);
    let beforeBalance = await cornerStone.balanceOf(user2);
    let nextPayment = await mortgageContract.getNextPaymentIndex({
      from: owner
    });
    console.log("nextPayment: " + nextPayment);
    let nextPaymentBalance = await mortgageContract.getNextPayment({
      from: owner
    });
    console.log("nextPaymentBalance: " + nextPaymentBalance);

    await cornerStone.mortgagePayment(mortgageAddress, MILL * 1.3, {
      from: owner
    });
    let balance = await cornerStone.balanceOf(user2);
    assert.equal(parseInt(paymentRefinance) + parseInt(balance), beforeBalance);
    await cornerStone.mortgagePayment(mortgageAddress, MILL * 1.3, {
      from: owner
    });
    balance = await cornerStone.balanceOf(user2);
    assert.equal(
      parseInt(paymentRefinance) * 2 + parseInt(balance),
      beforeBalance
    );
  });

  it(" check total money balance ", async () => {
    let balanceUser2 = await cornerStone.balanceOf(user2);
    let balanceUser1 = await cornerStone.balanceOf(user1);
    let balanceUser3 = await cornerStone.balanceOf(user3);
    let balanceOwner = await cornerStone.getOwnerBalance();
    let currentTotalFounds = bigInt(balanceUser2)
      .plus(bigInt(balanceUser1))
      .plus(bigInt(balanceUser3))
      .plus(bigInt(balanceOwner));
    console.log("totalSystemFounds:   " + bigInt(totalSystemFounds));
    console.log("balanceUser2:   " + bigInt(balanceUser2));
    console.log("balanceUser1:   " + bigInt(balanceUser1));
    console.log("currentTotalFounds:  " + bigInt(currentTotalFounds));
    let difference = bigInt(totalSystemFounds) - bigInt(currentTotalFounds);
    console.log(difference);
    assert.equal(difference, 0);
  });

  it(" check holdings returned to user1 ", async () => {
    let stoneCoinContract = StoneCoin.at(testStoneCoinAddress);
    userOwnedHoldings = await stoneCoinContract.getBalance(user2);
    console.log(userOwnedHoldings);
    assert.equal(parseInt(userOwnedHoldings), 140000);
  });
});
