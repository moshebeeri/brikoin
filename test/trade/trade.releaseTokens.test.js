...
contract("Trade Release Tokens", function(accounts) {
  let cornerStone;
  ...
  it("should release tokens", async () => {
    ...
    await stoneCoinContract.clearOwnerTokens({ from: user0 });
    ...
    assert.equal(user0Holdings, 0);
    assert.equal(user1Holdings, 10000);
  });

  it("should trade after tokens release ", async () => {
    await cornerStone.bid(testStoneCoinAddress, 10000, 1000000, 0, {
      from: user2
    });
