...
contract("Trade with bulk size", function(accounts) {
  let cornerStone;
  let owner;
  let taxContract;
  ...
  it("should create StoneCoin contract", async () => {
    ...
    await stoneCoinContract.setMaxBulkSize(5000, { from: user0 });
    await stoneCoinContract.setMinBulkSize(5000, { from: user0 });
    assert.equal(
  ...
  it("should bid ask trade with tax", async () => {
    await cornerStone.bid(testStoneCoinAddress, 5000, 1000000, 0, {
      from: user1
    });
  ...
  it("bid should fail", async () => {
    try {
      await cornerStone.bid(testStoneCoinAddress, 10000, 1500000, 0, {
        from: user2
      });
  ...
  it("ask should fail", async () => {
    try {
      await cornerStone.ask(testStoneCoinAddress, 10000, 1500000, 0, {
        from: user1
      });
      assert.equal(true, false);
    } catch (error) {
