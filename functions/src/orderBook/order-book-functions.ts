...
exports.inititalOffering = functions.database.ref(`server/operations/project/initialOffer`).onWrite(async (change) => {
...
async function addProjectAsk(projectId, amount, name, userId) {
    let project = <any> await getProjectById(projectId)
    const db = admin.database();
    let ask = {
        active: true,
        projectId: projectId,
    ...
    if(project.tradeMethod === 'auction') {
        let auctionRequest = <any> await getAuctionRequest(projectId)
...
exports.trade = functions.database.ref(`server/projects/events/orders`).onWrite(async (change) => {
    try {
        let resultChange = JSON.parse(JSON.stringify(change));
        let projectId = resultChange.after.projectId;
        let bidsOrders = await getActiveBids(projectId);
        let asksOrders = await getActiveAsks(projectId);
...
function calculateOrders(projectId, bidsOrders,asksOrders) {
    try {
        ...
        projectOrders.forEach(order => {
            let limitOrder = new LimitOrder(order.orderId, order.side, order.price, order.size);
...
async function moveToHistory(projectId,orderId,side,size){
    try {
        const db = admin.database();
        let order = await getOrder(projectId, orderId, side);
