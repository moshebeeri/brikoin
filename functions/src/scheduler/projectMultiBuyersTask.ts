...
export async function triggerProjectMultiBuyerCondition(task) {
	return true;
}

export async function triggerProjectMultiBuyerRun(task) {
	...
	console.log('BID');
	let response = <any>await projectBidRequest(userAccount.accountId, order.project, order.amount, order.price, 0);
