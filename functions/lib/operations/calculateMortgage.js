...
function calculateMortgage(options) {
    ...
    function calculateLoanAmount(mortgage) {
        ...
        if (typeof mortgage.downPayment === 'string') {
            let downPercent = mortgage.downPayment.substr(0, mortgage.downPayment.indexOf('%'));
    ...
    function calculateExtraPayment(mortgage, loanMonth) {
        let totalExtra = 0;
        if (mortgage.extras) {
            for (let i in mortgage.extras) {
                let extra = mortgage.extras[i];
    ...
    function calculateInterestRate(mortgage, loanMonth) {
        ...
        let armInterestRate = mortgage.interestRate + mortgage.adjustInitialCap;
        if (loanMonth > mortgage.adjustFixedRateMonths + 1) {
            for (let i = mortgage.adjustFixedRateMonths + mortgage.adjustIntervalMonths; i <= loanMonth; i += mortgage.adjustIntervalMonths) {
    ...
    function calculateMonthlyPayment(loanAmount, loanTermMonths, interestRate) {
        let monthlyInterestRate = aprToMonthlyInterest(interestRate);
        let monthlyPayment = loanAmount * (monthlyInterestRate * Math.pow(1 + monthlyInterestRate, loanTermMonths)) /
    ...
    function calculateAmortizations(mortgage) {
        ...
        // to response objects.
        let remainingLoanAmountInCents = mortgage.loanAmount * 100;
        let loanAmountInCents = mortgage.loanAmount * 100;
        let monthlyPropertyTaxInCents = calculatePropertyTax(mortgage);
        let amortizations = [];
        let previousAmortization;
        let loanMonth = 0;
        let loanYear = 1;
        let loanYearRollUpSummary = {};
        let currentInterestRate = calculateInterestRate(mortgage, 1);
        let currentMonthlyPaymentInCents = calculateMonthlyPayment(remainingLoanAmountInCents, mortgage.loanTermMonths, currentInterestRate);
        ...
        while (remainingLoanAmountInCents >= 1) {
            ...
            amortization.paymentDate = new Date(mortgage.startDate);
            amortization.paymentDate.setMonth(amortization.paymentDate.getMonth() + loanMonth);
        ...
        let additionalFieldsToProcess = ['scheduledMonthlyPayment', 'remainingLoanBalnce'];
    ...
    let mortgage = initOptions(options);
...
//# sourceMappingURL=calculateMortgage.js.map