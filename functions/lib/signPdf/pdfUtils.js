...
const cloudConvert = new cloudconvert_1.default(apiKey);
const fileBucket = functions.config().file.bucket;
function convertFile(fileName, filePath, sourceFormat, destFormat, metadata) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let job = yield cloudConvert.jobs.create({
            tasks: {
                'file-upload': {
        ...
        yield bucket.file(`${filePath}/${fileName}`).download({ destination: tempFilePath });
        ...
        yield cloudConvert.tasks.upload(uploadTask, inputFile);
        console.log(`waiting for job to finish`);
        job = yield cloudConvert.jobs.wait(job.id); // Wait for job completion
        console.log(`creating convert job`);
        console.log(`convert task id  ${uploadTask.id}`);
        job = (yield cloudConvert.jobs.create({
            tasks: {
                'convert-my-file': {
                    operation: 'convert',
                    input: uploadTask.id,
        ...
        job = yield cloudConvert.jobs.wait(job.id);
        ...
        let downloadPath = yield downloadFile(file.filename, file.url);
        console.log(`write file to bucket`);
        console.log(`file application/${destFormat}`);
        yield bucket.upload(downloadPath, {
