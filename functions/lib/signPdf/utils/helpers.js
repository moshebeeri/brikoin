...
// import fs from 'fs'
const fs = require("fs");
const signpdf_1 = require("./signpdf");
const SignPdfError_1 = require("./SignPdfError");
const SignPdfError_2 = __importDefault(require("./SignPdfError"));
const path = require('path');
const pdfobject_1 = __importStar(require("./pdfobject"));
const os = require('os');
 ...
 * @returns {Buffer}
 */
exports.removeTrailingNewLine = (pdf) => {
    ...
    if (lastLine !== '\n%%EOF') {
        throw new SignPdfError_2.default('A PDF file must end with an EOF line.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    return pdf;
};
/**
 ...
 * @returns {object}
 */
exports.readRefTable = (pdf, position) => {
    const offsetsMap = new Map();
    let refTable = pdf.slice(position);
    if (refTable.indexOf('xref') !== 0) {
        throw new SignPdfError_2.default('Unexpected cross-reference table format.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
...
const getIndexFromRef = (refTable, ref) => {
    ...
    if (!refTable.offsets.has(index)) {
        throw new SignPdfError_2.default(`Failed to locate object "${ref}".`, SignPdfError_1.ERROR_TYPE_PARSE);
    }
    return index;
};
/**
 ...
 * @returns {object}
 */
exports.findObject = (pdf, refTable, ref) => {
...
exports.readPdf = (pdf) => {
    ...
    if (trailer.lastIndexOf('/Prev') !== -1) {
        throw new SignPdfError_2.default('Incrementally updated PDFs are not yet supported.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    ...
    const refTable = exports.readRefTable(pdf, xRefPosition);
    const root = exports.findObject(pdf, refTable, rootRef).toString();
    if (root.indexOf('AcroForm') !== -1) {
        throw new SignPdfError_2.default('The document already contains a form. This is not yet supported.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    if (refTable.maxOffset > refTable.tableOffset) {
        throw new SignPdfError_2.default('Ref table is not at the end of the document. This document can only be signed in incremental mode.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    return {
        xref: refTable,
        rootRef,
 ...
 * @returns {object}
 */
exports.addSignaturePlaceholder = ({ pdf, reason, signatureLength = exports.DEFAULT_SIGNATURE_LENGTH }) => {
    ...
    const signature = pdf.ref({
        ...
        ByteRange: [
            0,
            signpdf_1.DEFAULT_BYTE_RANGE_PLACEHOLDER,
            signpdf_1.DEFAULT_BYTE_RANGE_PLACEHOLDER,
            signpdf_1.DEFAULT_BYTE_RANGE_PLACEHOLDER
        ],
...
const getPagesDictionaryRef = (info) => {
    ...
    if (match === null) {
        throw new SignPdfError_2.default('Failed to find the pages descriptor. This is probably a problem in node-signpdf.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    return match[1];
};
const createBufferPageWithAnnotation = (pdf, info, pagesRef, widget) => {
    const pagesDictionary = exports.findObject(pdf, info.xref, pagesRef).toString();
    if (pagesDictionary.indexOf('/Annots') !== -1) {
        throw new SignPdfError_2.default('There already are /Annots described. This is not yet supported', SignPdfError_1.ERROR_TYPE_PARSE);
    }
...
const getPageRef = (pdf, info) => {
    ...
    const pagesDictionary = exports.findObject(pdf, info.xref, pagesRef);
 ...
 * @param {Buffer} pdf
 */
exports.plainAdd = (pdfBuffer, { reason, signatureLength = exports.DEFAULT_SIGNATURE_LENGTH }) => {
    let pdf = exports.removeTrailingNewLine(pdfBuffer);
    const info = exports.readPdf(pdf);
    ...
    const pdfKitMock = {
        ref: (input) => {
            ...
            return new pdfobject_1.PDFReferenceMock(info.xref.maxIndex);
        },
        page: {
            dictionary: new pdfobject_1.PDFReferenceMock(pageIndex, {
                data: {
                    Annots: []
                }
            })
    ...
    fs.createWriteStream(beforePath).end(pdf);
    const { form, widget } = exports.addSignaturePlaceholder({
        pdf: pdfKitMock,
        reason,
        signatureLength
    });
    ...
    fs.createWriteStream(afterPath).end(pdf);
    return pdf;
};
exports.extractSignature = (pdf) => {
    const byteRangePos = pdf.indexOf('/ByteRange [');
    if (byteRangePos === -1) {
        throw new SignPdfError_2.default('Failed to locate ByteRange.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
    const byteRangeEnd = pdf.indexOf(']', byteRangePos);
    if (byteRangeEnd === -1) {
        throw new SignPdfError_2.default('Failed to locate the end of the ByteRange.', SignPdfError_1.ERROR_TYPE_PARSE);
    }
