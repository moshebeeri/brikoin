...
exports.removeTrailingNewLine = (pdf) => {
    ...
    if (lastLine !== '\n%%EOF') {
        throw new SignPdfError_1.default('A PDF file must end with an EOF line.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    return pdf;
};
/**
...
exports.readRefTable = (pdf, position) => {
    ...
    if (refTable.indexOf('xref') !== 0) {
        throw new SignPdfError_1.default('Unexpected cross-reference table format.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
...
const getIndexFromRef = (refTable, ref) => {
    ...
    if (!refTable.offsets.has(index)) {
        throw new SignPdfError_1.default(`Failed to locate object "${ref}".`, SignPdfError_2.ERROR_TYPE_PARSE);
    }
    return index;
};
/**
...
exports.readPdf = (pdf) => {
    ...
    if (trailer.lastIndexOf('/Prev') !== -1) {
        throw new SignPdfError_1.default('Incrementally updated PDFs are not yet supported.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    ...
    if (root.indexOf('AcroForm') !== -1) {
        throw new SignPdfError_1.default('The document already contains a form. This is not yet supported.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    if (refTable.maxOffset > refTable.tableOffset) {
        throw new SignPdfError_1.default('Ref table is not at the end of the document. This document can only be signed in incremental mode.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    return {
        xref: refTable,
        rootRef,
...
const getPagesDictionaryRef = (info) => {
    ...
    if (match === null) {
        throw new SignPdfError_1.default('Failed to find the pages descriptor. This is probably a problem in node-signpdf.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    return match[1];
};
const createBufferPageWithAnnotation = (pdf, info, pagesRef, widget) => {
    ...
    if (pagesDictionary.indexOf('/Annots') !== -1) {
        throw new SignPdfError_1.default('There already are /Annots described. This is not yet supported', SignPdfError_2.ERROR_TYPE_PARSE);
    }
...
exports.extractSignature = (pdf) => {
    ...
    if (byteRangePos === -1) {
        throw new SignPdfError_1.default('Failed to locate ByteRange.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
    const byteRangeEnd = pdf.indexOf(']', byteRangePos);
    if (byteRangeEnd === -1) {
        throw new SignPdfError_1.default('Failed to locate the end of the ByteRange.', SignPdfError_2.ERROR_TYPE_PARSE);
    }
